...
export function setupMessageHandlers(
  ...
  const { isWalletReady, onApproveRequest, signTransaction, onError } = options;
  ...
  const handler = (event: MessageEvent<unknown>) => {
    ...
    switch (type) {
      ...
      case MessageType.WALLET_SIGN: {
        ...
        if (!isWalletReady()) {
          sendToParent(createSignError(requestId, "Wallet not ready"));
          return;
        }
        ...
        onApproveRequest(signRequest)
          .then((approved) => {
            ...
            if (!approved) {
              sendToParent(createSignError(requestId, "User rejected"));
              return;
            }
          ...
          .then((signature) => {
            if (signature) {
              sendToParent(createSignResponse(requestId, signature));
            }
          })
          .catch((err) => {
            const t = pendingTimeouts.get(requestId);
