...
export function SendSolForm({
  ...
  const handleSend = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!Number.isFinite(parsedAmount) || parsedAmount < MIN_SOL_AMOUNT) {
        throw new Error(`Amount must be at least ${MIN_SOL_AMOUNT} SOL`);
      }
  ...
  return (
    <Card className="w-full max-w-lg">
      <CardHeader>
        ...
        <CardDescription>
          Create a transfer, approve in the embedded wallet iframe, and submit
          to Solana devnet.
        </CardDescription>
      </CardHeader>
      <CardContent>
        <form onSubmit={handleSend} className="space-y-4">
          <div className="space-y-2">
            <label htmlFor="sol-recipient" className="text-sm font-medium">
              Recipient address
            </label>
            <input
              id="sol-recipient"
            ...
            {recipient.trim().length > 0 &&
              !isLikelySolanaAddress(recipient) && (
                <p className="text-xs text-destructive">
                  Enter a valid Solana base58 address (32-44 chars).
                </p>
              )}
          </div>

          <div className="space-y-2">
            <label htmlFor="sol-amount" className="text-sm font-medium">
              Amount (SOL)
            </label>
            <input
              id="sol-amount"
